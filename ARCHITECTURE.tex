...
\file{src/styles/themes/\{foodwars,naruto,free\}.css} &
...
aesthetic), Free! (an alternate, aquatic-toned) --- controlled by one slider that
...
(there's no ``prefers-free'' signal from the OS) --- they need an
explicit switch. The mechanism is a \texttt{data-theme} HTML attribute on
\texttt{<html>}, and one CSS file per theme
(\texttt{src/styles/themes/\{foodwars,naruto,free\}.css}), each
...
visitor who picked Free! would see a flash of Food Wars (the CSS
...
(Food Wars), or a rippling current with bubble nodes (Free!) ---
\texttt{SPEC.md} \S4. Each is its own self-contained component
(\texttt{src/components/connectors/\{Tree,Butter,Water\}Connector.astro})
...
leaf-spiral/splash-ripple decoration behind the section) uses the
